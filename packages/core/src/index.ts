...
import type { WeatherTheme, AQIData } from '@skye/types'

export function cn(...inputs: ClassValue[]) {
  return twMerge(clsx(inputs))
}

// WMO weather codes (Open-Meteo standard, 0-99)
export function getWeatherTheme(conditionId: number, dt: number, sunrise: number, sunset: number): WeatherTheme {
  ...
  if (conditionId === 95 || conditionId === 96 || conditionId === 99) return 'stormy'
  if ((conditionId >= 51 && conditionId <= 67) || (conditionId >= 80 && conditionId <= 82)) return 'rainy'
  if (conditionId >= 71 && conditionId <= 77) return 'snowy'
  if (conditionId === 45 || conditionId === 48) return 'foggy'
  if (conditionId === 0 || conditionId === 1) return isNight ? 'clear-night' : 'clear-day'
  if (conditionId === 2) return isNight ? 'partly-cloudy-night' : 'partly-cloudy-day'
  return 'cloudy'
}
